...
class HandEyeCalibration:
    def __init__(self):
        ...
        self.calib_result_pub = rospy.Publisher(
            "/calibration/result", Bool, queue_size=10)

        # 订阅触发校准的标志位
        self.trigger_sub = rospy.Subscriber(
            '/trigger_calibration', Bool, self.trigger_callback)
    ...
    def get_image_pose(self):
        while True:
            if self.cv_image is not None:
                deep_copied_image = copy.deepcopy(self.cv_image)
                ...
                # 检测棋盘格角点
                ret, corners = cv2.findChessboardCornersSB(gray, self.board_size, flags=cv2.CALIB_CB_EXHAUSTIVE +
                ...
                if ret:
                    ...
                    if self.trigger_calibration:
                        ...
                        if current_pose is not None:
                            R_gripper2base, t_gripper2base = self.pose_to_transform(
                                current_pose)
                            ...
                            ret, rvec, tvec = cv2.solvePnP(
                                obj_points, corners2, self.camera_matrix, self.dist_coeffs)
    ...
    def print_calibration_results(self, R_cam2gripper, t_cam2gripper):
        ...
        rospy.loginfo(
            f"[{t_cam2gripper[0][0]}, {t_cam2gripper[1][0]}, {t_cam2gripper[2][0]}]")
