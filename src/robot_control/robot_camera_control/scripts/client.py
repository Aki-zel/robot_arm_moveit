...
def color_detect_client(run):
    rospy.wait_for_service('color_detect')
    try:
        # 创建服务代理
        detect_service = rospy.ServiceProxy('color_detect', Hand_Catch)
        ...
        if resp.detect_image:
            print("Received image message successfully")
            bridge = CvBridge()
            ...
            print("Did not receive image message")

        print("Labels:", resp.labels)
        print("Positions:", resp.positions)
        return resp
