...
class GraspGenerator:
    ...
    def generate(self):
        ...
        if len(grasps) != 0:
            ...
            pos_x = np.multiply(
                grasps[0].center[1] + self.cam_data.top_left[1] - self.camera_info.K[2],
                pos_z / self.camera_info.K[0],
            )
            pos_y = np.multiply(
                grasps[0].center[0] + self.cam_data.top_left[0] - self.camera_info.K[5],
            ...
            target_position = (
                np.dot(camera2robot[0:3, 0:3], target) + camera2robot[0:3, 3:]
            )
