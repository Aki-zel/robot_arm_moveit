...
class GraspGenerator:
    def __init__(self, saved_model_path, cam_id, visualize=False):
        ...
        for file_path in [self.grasp_request, self.grasp_available, self.grasp_pose]:
            if not os.path.exists(file_path):
                np.save(file_path, np.zeros((1,)))  # 这里用一个简单的零数组作为初始数据
        if visualize:
    ...
    def run(self):
        print("start")
        while True:
            grasp_request_state = np.load(self.grasp_request)
            if grasp_request_state.size > 0:
                self.generate()
